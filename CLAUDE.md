...
**Acceso directo al informe apenas se finaliza una inspección (falta hoy):** al presionar "Finalizar revisión" (§2.5) y guardar exitosamente, el supervisor debe quedar de inmediato frente a un botón claro para **generar el informe y enviarlo por correo** — no debe tener que ir a buscar el ticket manualmente en "Mis inspecciones" para encontrar esa opción. Al terminar de guardar la revisión, muestra una pantalla de confirmación (o redirige directo a `/tickets/[id]/report`, §4) con un botón del tipo "Generar informe y enviar por correo" que lleve a la vista del informe con el selector de destinatarios (§4.1) ya listo para usar. Además, en la tabla de "Mis inspecciones" del supervisor, agrega en la columna "Acciones" un acceso directo al informe de cada ticket (por ejemplo un botón/ícono "Informe"), junto al botón "Ver" que ya existe — no debe quedar solo alcanzable entrando primero al detalle del ticket.

**Corrección — el detalle del ticket (`/tickets/[id]`) también debe permitir enviar el correo, no solo verlo:** hoy, al entrar a un ticket desde el listado (botón "Ver"), la pantalla de detalle solo ofrece "Ver informe" (que solo muestra/descarga el PDF) — agrega ahí mismo la opción de **enviar el informe por correo** (el mismo selector de destinatarios de §4.1), no solo desde la pantalla del informe. Puede ser un botón "Enviar por correo" junto a "Ver informe" en esa misma cabecera, o abrir directamente la vista de §4 con el selector listo — cualquiera de las dos formas es válida, mientras el supervisor no tenga que primero entrar a "Ver informe" para recién ahí encontrar cómo enviarlo.
