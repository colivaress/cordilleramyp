...
- **Tabla "Usuarios":** lista todo `personal` — Nombre, Apellido, Correo, Teléfono, Fecha de nacimiento, Rol (badge Supervisor/Administrador), Estado (Activo / Inactivo / "Invitación pendiente" si aún no ha iniciado sesión nunca), y Acciones (editar, activar/desactivar, reenviar invitación si sigue pendiente). "Editar" abre el mismo formulario que "Agregar usuario" (ver abajo) precargado con los datos actuales, para poder corregir cualquier campo — incluido completar el Apellido o la Fecha de nacimiento de usuarios ya creados que todavía no los tengan.
  - **Corrección — la columna "Fecha de nacimiento" se saca de esta tabla:** no debe mostrarse como columna en el listado de "Usuarios" — saca la columna completa (encabezado y celdas), no la dejes vacía. El dato **no se elimina de la base ni del formulario**: sigue siendo un campo obligatorio en "Agregar usuario"/"Editar" (más abajo) y se sigue guardando igual en `personal.fecha_nacimiento` — esta corrección es solo para no mostrarlo en la tabla.
...
**Corrección posterior de texto — el título del informe ya no debe llevar "Cordillera M&P" adelante, queda solo "Informe de Inspección":** hoy el encabezado (en pantalla y en el PDF) muestra el título como "Cordillera M&P — Informe de Inspección" — saca el prefijo "Cordillera M&P —" en todos los lugares donde aparezca ese título (el encabezado visible del informe, el PDF generado para correo/WhatsApp §4.1/§4.2, y el título de la pestaña/página si repite ese mismo texto). El título queda como texto plano **"Informe de Inspección"**, sin el nombre de la empresa adelante — la marca ya queda representada por el logo grande que ahora va al lado derecho de ese mismo encabezado (corrección de §8, más abajo), no hace falta repetirla en el texto. Esto no afecta al `title`/asunto que arma la Web Share API en §4.2 (`'Informe de Inspección — {transporte} {patente_camion}'`) ni al del correo (§4.1) — esos son textos distintos (metadata del compartir/asunto del correo), no el título visible del documento.
...
**Corrección de rendimiento — "Enviar por correo" se demora cerca de 1 minuto en completar el envío, hay que reducir ese tiempo al mínimo posible:** se probó en vivo y el botón "Enviar por correo" (generar el PDF + mandarlo por SMTP) tarda demasiado. No apliques un solo cambio a ciegas — primero mide dónde se va el tiempo, y después optimiza la parte que realmente lo explica:

- **Primero, perfila el flujo real:** agrega logs con timestamps (o `console.time`/`console.timeEnd`) alrededor de cada etapa del envío — descarga/lectura de las fotos de evidencia, generación del PDF, y el envío SMTP en sí — para confirmar en los logs del servidor cuál de las tres se lleva la mayor parte del minuto, en vez de asumir cuál es. Con esa medición real, prioriza la optimización de abajo que corresponda.
- **Sospecha principal — fotos de evidencia sin comprimir:** una foto tomada con el celular (§2.8, captura de cámara) puede pesar varios MB sin comprimir; si el checklist tiene varios ítems "No conformes" con foto, el PDF puede estar descargando e incrustando varias fotos pesadas sin redimensionar ni comprimir — eso es lento tanto para descargarlas desde Storage como para que la librería de PDF las procese. Corrige esto en dos frentes:
  1. **Redimensiona/comprime la foto al momento de subirla** (§2.4/§2.8, cuando se captura o selecciona), antes de subirla al bucket `fallas` — por ejemplo a un ancho máximo de 1600px y calidad ~80% (usa `canvas`/`createImageBitmap` en el cliente, o una librería liviana tipo `browser-image-compression`). Esto además acelera la carga de esa foto en cualquier otra pantalla que la muestre (lightbox de `/tickets/[id]`, §2.4), no solo el PDF.
  2. **Al armar el PDF, descarga todas las fotos que necesita en paralelo** (`Promise.all`, no una por una en serie) en vez de esperar cada descarga antes de pedir la siguiente.
- **Si la generación del PDF usa Puppeteer/Playwright** (renderizando el HTML del informe con un navegador headless, una de las opciones que dejaba abierta la spec original de §4.1): confirma que no se esté lanzando una instancia de navegador nueva y pesada en cada envío si el hosting lo permite reutilizar, y evalúa si conviene migrar a `@react-pdf/renderer` (la otra opción que ya contemplaba la spec) — genera el PDF sin levantar un navegador headless completo, mucho más rápido en un entorno serverless como Vercel, donde además Puppeteer necesita paquetes adicionales (`@sparticuz/chromium`) para funcionar bien y sigue siendo lento por el cold start del navegador.
- **Reutiliza el transporter de `nodemailer`** (créalo una sola vez a nivel de módulo/archivo, no dentro de cada request) para no repetir el handshake TLS con Gmail en cada envío — este paso por sí solo no explica un minuto completo, pero suma.
- **No regeneres el mismo PDF dos veces si el usuario ya lo generó en esa misma visita a la página** (por ejemplo si primero prueba "Enviar por WhatsApp", §4.2, y después "Enviar por correo") — cachea el PDF ya generado en el estado de esa página/sesión y reutilízalo para el segundo envío en vez de volver a construirlo desde cero.
- **Mientras se reduce el tiempo real, mejora también la percepción:** muestra un indicador de carga claro en el botón "Enviar por correo" mientras dura el proceso (por ejemplo, deshabilita el botón y cambia su texto a "Generando informe y enviando…" con un spinner) — esto no reemplaza la optimización de arriba, pero evita que el supervisor sienta que la app quedó colgada durante la espera.
- **Verifica en vivo con un ticket real que tenga varias fotos de evidencia** (el caso más pesado) que el tiempo total de "Enviar por correo" bajó de forma notoria respecto al minuto actual, y que el correo sigue llegando con el PDF adjunto completo y legible.
...
- ~~Logo de Cordillera M&P~~ — **resuelto: el usuario entregó el logo real de la empresa.** Ya está en el repo, en `public/logo-cordillera-mp.png` (imagen PNG, fondo blanco, 2816×1408 px — círculo naranjo con el camión y "Cordillera M&P", más "Encarpe y Amarre / Nacionales e Internacionales" debajo). Intégralo en estos lugares, reemplazando cualquier texto/placeholder que hiciera de logo hasta ahora:
  ...
  - **Header de la app (nav superior), en las dos vistas (administrador y supervisor) y también en la pantalla de login:** muestra la imagen del logo junto al nombre "Cordillera M&P" (o en vez del texto, a criterio de la implementación, mientras quede prolijo) — manteniendo la proporción original de la imagen, `next/image` con `width`/`height` fijos o `object-fit: contain`. Revisa que se vea bien tanto en escritorio como en la vista de celular (§6, corrección de responsive ya hecha para el nombre del usuario).
    ...
    - **Corrección — en la pantalla de login el logo también queda chico, agrándalo aparte:** la pantalla de login no usa el nav superior (es una tarjeta centrada con el logo arriba de "Iniciar sesión", sin el resto de los elementos del header — nombre de usuario, badge de rol, "Salir", que ahí no existen porque todavía no hay sesión), así que el tamaño de arriba (88–112px) no necesariamente aplica igual — ajusta el logo de esa tarjeta de login específicamente para que se vea protagonista, notoriamente más grande que ahora (por ejemplo un ancho/alto en el rango de 140–170px, manteniendo la proporción original de la imagen), sin que la tarjeta quede desproporcionada ni el logo se corte. Verifica que el resto de la tarjeta (título "Iniciar sesión", subtítulo, campos, botón "Ingresar", enlaces) siga centrado y con buen espaciado alrededor del logo agrandado.
  ...
  - **Firma del correo (§4.1):** agrega el logo también en el cuerpo del correo, por ejemplo debajo de la firma ("Atentamente, {nombre} {apellido}, Supervisor de Encarpe") — como es un correo HTML enviado por SMTP (nodemailer), **no lo references por una URL pública** (la app puede no estar desplegada o el logo puede no ser accesible desde internet); en su lugar, adjúntalo como **imagen embebida (`cid`)**: agrega el archivo como `attachments: [{ filename: 'logo-cordillera-mp.png', path: 'public/logo-cordillera-mp.png', cid: 'logo-cordillera-mp' }]` en la llamada de `nodemailer`, y referencia esa imagen en el HTML del cuerpo con `<img src="cid:logo-cordillera-mp" ... />` — así se ve siempre, sin depender de que el archivo esté accesible por internet.
    - **Corrección — el logo de la firma del correo debe alinearse a la izquierda, con el nombre de quien envía. Sigue sin corregirse, verifícalo de nuevo en vivo, no asumas que ya quedó bien:** se pidió esta corrección una vez y se probó de nuevo enviando un correo real — el logo **sigue** viéndose desalineado respecto al bloque de firma ("Atentamente, {nombre}, Supervisor de Encarpe"), que está alineado a la izquierda. No repitas el mismo cambio a ciegas si ya lo intentaste — revisa el HTML final que realmente sale del correo (no solo el JSX/template en el código):
      - **Causa probable si el cambio anterior no tuvo efecto:** el `<img>` puede estar dentro de un contenedor que sigue centrando su contenido (por ejemplo un `<td align="center">`, un `<div style="text-align:center">` heredado de una fila de la tabla de la firma, o el propio `<table>` del cuerpo del correo con `align="center"` en una celda) — un `margin`/`display:block` puesto solo en el `<img>` no alcanza si el elemento que lo contiene sigue centrando. Revisa **todos los contenedores** desde el `<img>` hacia arriba hasta el bloque de la firma, no solo la imagen.
      - Muchos clientes de correo (Gmail incluido) ignoran o normalizan CSS no inline — confirma que la alineación esté en atributos/estilos **inline** en cada nivel relevante (`<td align="left">` o `style="text-align:left"` en el contenedor, y `style="display:block; margin:12px 0 0 0;"` en el `<img>`), no en una clase o `<style>` que el cliente de correo pueda estar descartando.
      - **Verifica mandando un correo de prueba real y abriéndolo en Gmail** (como en la captura) después del cambio — no lo des por resuelto solo leyendo el código. El logo debe quedar pegado al mismo borde izquierdo que "Atentamente," y el nombre, no centrado ni corrido a la derecha.
