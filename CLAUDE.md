...
- **Cuerpo del correo — corrección: pásalo a HTML con una tabla, formato formal y profesional** (reemplaza tanto el texto plano anterior como cualquier otro texto genérico que haya quedado, por ejemplo si el asunto del correo todavía dice algo como "Se realizó la inspección técnica del camión. Se adjunta checklist" — unifica todo a esta redacción). Envía el correo como HTML (`html:` en `nodemailer`, no `text:` plano) con esta estructura:
  ...
  La plantilla real (ya implementada) vive en `src/lib/mensajes.ts` — `construirCuerpoInforme` es la fuente de verdad de este cuerpo de correo, junto a `construirCorreoVencimientoAdmin` y `construirCorreoVencimientoExterno` (las otras dos plantillas de correo del proyecto, §3.2). No copiar el HTML acá — leer el código.
...
**`auth.getUser()` nunca se reemplaza por la sesión local, ni siquiera "para ganar unos ms".** `supabase.auth.getUser()` es una llamada de red al servidor de Auth — valida el token contra Supabase, no lee el JWT de la cookie local — y cuesta entre 250 y 600ms cada vez que se llama (medido; ver PR #37). `auth.getSession()` (o decodificar el JWT a mano) es más rápido porque confía en lo que diga la cookie sin verificarlo contra el servidor, así que se puede falsificar. Todo `requireRol()`/`getSesion()` del sistema (admin vs. supervisor, y por extensión cada política RLS que asume que `auth.uid()` es confiable) depende de que esa identidad esté realmente validada — cambiar a la versión local rompería esa garantía, no solo la haría "un poco menos segura". El arreglo correcto cuando `getUser()` sale caro por llamarse varias veces en el mismo request (típico: una vez en un layout, otra en cada page.tsx que lo anida) es deduplicarlo con `React.cache()` — una sola llamada real por request, compartida — nunca llamarlo menos veces confiando en una fuente no verificada. Ver el comentario en `src/lib/auth.ts` (`getSesion`).

**`router.refresh()` después de `router.push()` duplica la carga completa de la ruta nueva** — no es gratis "por si acaso". Si la Server Action que se acaba de llamar ya invalida los datos correctos con `revalidatePath`/`revalidateTag` (lo normal en este proyecto), un `router.refresh()` del lado del cliente no invalida nada que no esté invalidado ya — solo dispara una segunda request completa (con su propia validación de sesión, sus propias queries) a una ruta que además está a punto de reemplazarse por la navegación de `router.push()`. Antes de agregar uno, confirma que la Server Action no tenga ya su propio `revalidatePath` para lo que se necesita refrescar; si lo tiene, el `router.refresh()` sobra.
